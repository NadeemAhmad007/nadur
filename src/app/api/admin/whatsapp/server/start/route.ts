...
export async function POST() {
  ...
  try {
    ...
    return NextResponse.json({
      error: 'Docker is not available on this server. Run the container manually on your Docker host.',
      needs_docker: true,
    }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: 'OpenWA server started' });
  } catch (error: any) {
